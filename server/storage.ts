import { 
  users, userProfiles, monetizationOpportunities, messages, connections,
  type User, type InsertUser, 
  type UserProfile, type InsertUserProfile,
  type MonetizationOpportunity, type InsertMonetizationOpportunity,
...
export interface IStorage {
  ...
  saveOpportunity(opportunity: InsertMonetizationOpportunity): Promise<MonetizationOpportunity>;
  ...
  updateConnectionStatus(connectionId: number, status: string): Promise<Connection | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Monetization opportunities
  async saveOpportunity(opportunity: InsertMonetizationOpportunity): Promise<MonetizationOpportunity> {
    const result = await db.insert(monetizationOpportunities).values(opportunity).returning();
    return result[0];
  }

  async getUserOpportunities(userId: number): Promise<MonetizationOpportunity[]> {
    return db.select().from(monetizationOpportunities).where(eq(monetizationOpportunities.userId, userId));
  }

  async getSharedOpportunities(): Promise<MonetizationOpportunity[]> {
    return db.select().from(monetizationOpportunities).where(eq(monetizationOpportunities.shared, true));
  }
  ...
  async updateConnectionStatus(connectionId: number, status: string): Promise<Connection | undefined> {
