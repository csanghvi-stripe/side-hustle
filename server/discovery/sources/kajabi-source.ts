...
export class KajabiSource extends BaseOpportunitySource {
  ...
  public async getOpportunities(
    ...
    logger.info(`Fetching opportunities from Kajabi for skills: ${skills.join(', ')}`);
    ...
    if (cached && Date.now() - cached.timestamp < this.cacheTimeMs) {
      logger.info(`Using cached Kajabi opportunities (${cached.data.length} items)`);
      return cached.data;
    }
   ...
   */
  private async getKajabiOpportunities(skills: string[]): Promise<RawOpportunity[]> {
    try {
      const opportunities: RawOpportunity[] = [];
      
      // Define relevant skill categories
      const techSkills = ['programming', 'web development', 'design', 'data science', 'AI'];
      const businessSkills = ['marketing', 'entrepreneurship', 'sales', 'consulting', 'coaching'];
      const creativeSkills = ['writing', 'design', 'video production', 'content creation'];
      ...
      if (userHasCreativeSkills || userHasWellnessSkills) {
        opportunities.push(this.createDigitalProductsBusinessOpportunity(skills));
      }
      
      return opportunities;
    } catch (error) {
      logger.error(`Error generating Kajabi opportunities: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  private createGeneralKajabiOpportunity(): RawOpportunity {
    ...
    return {
      ...
      requiredSkills: ['expertise in a subject', 'content creation', 'business sense'],
  ...
  private createMembershipOpportunity(): RawOpportunity {
    ...
    return {
      ...
      requiredSkills: ['consistent content creation', 'community management', 'teaching ability'],
      niceToHaveSkills: ['engagement strategies', 'retention marketing', 'live events'],
      estimatedIncome: {
        min: 5000,
        max: 100000,
        timeframe: 'month'
  ...
  private createTechBusinessOpportunity(skills: string[]): RawOpportunity {
    ...
    return {
      ...
      niceToHaveSkills: ['project-based instruction', 'community management', 'career guidance'],
      estimatedIncome: {
        min: 5000,
        max: 100000,
        timeframe: 'month'
  ...
  private createCoachingBusinessOpportunity(skills: string[]): RawOpportunity {
    ...
    return {
      ...
      requiredSkills: ['coaching expertise', 'business acumen', 'problem solving'],
      niceToHaveSkills: ['group facilitation', 'curriculum design', 'client management'],
      estimatedIncome: {
        min: 5000,
        max: 50000,
        timeframe: 'month'
  ...
  private createDigitalProductsBusinessOpportunity(skills: string[]): RawOpportunity {
    const title = 'Create a Digital Products Empire on Kajabi';
    
    // Find matching skills for personalization
    const relevantSkills = [
      'writing', 'design', 'video production', 'content creation',
      'fitness', 'nutrition', 'yoga', 'meditation', 'coaching'
    ];
    const matchingSkills = skills.filter(skill => 
      relevantSkills.some(relevantSkill => skill.toLowerCase().includes(relevantSkill.toLowerCase()))
    );
    ...
    return {
      ...
      niceToHaveSkills: ['sales funnel design', 'email marketing', 'audience building'],
      estimatedIncome: {
        min: 5000,
        max: 100000,
        timeframe: 'month'
