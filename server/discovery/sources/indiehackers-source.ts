...
export class IndieHackersSource extends BaseOpportunitySource {
  constructor() {
    super("IndieHackers", "indiehackers", "https://www.indiehackers.com");
  }
  ...
  public async getOpportunities(
    ...
    logger.info(
      `[${this.id}] Getting opportunities from IndieHackers for skills: ${skills.join(", ")}`,
    );
    ...
    if (cached && Date.now() - cached.timestamp < this.cacheTimeMs) {
      logger.info(
        `Using cached IndieHackers opportunities (${cached.data.length} items)`,
      );
      return cached.data;
    }
   ...
   */
  private async getIndieHackersOpportunities(
    skills: string[],
  ): Promise<RawOpportunity[]> {
    try {
      ...
      const userHasContentSkills = skills.some((skill) =>
        contentSkills.some((contentSkill) =>
          skill.toLowerCase().includes(contentSkill.toLowerCase()),
        ),
      );
      ...
      logger.error(
        `Error generating IndieHackers opportunities: ${error instanceof Error ? error.message : String(error)}`,
      );
      return [];
    }
  }
