...
import { DiscoveryPreferences, RawOpportunity, Resource, SuccessStory } from '../types';
...
export class MavenSource extends BaseOpportunitySource {
  ...
  public async getOpportunities(
    ...
    logger.info(`Fetching opportunities from Maven for skills: ${skills.join(', ')}`);
    ...
    if (cached && Date.now() - cached.timestamp < this.cacheTimeMs) {
      logger.info(`Using cached Maven opportunities (${cached.data.length} items)`);
      return cached.data;
    }
   ...
   */
  private async getMavenOpportunities(skills: string[]): Promise<RawOpportunity[]> {
    try {
      const opportunities: RawOpportunity[] = [];
      
      // Define relevant skill categories
      const techSkills = ['programming', 'web development', 'data science', 'machine learning', 'AI', 'python', 'javascript', 'react', 'node.js'];
      const businessSkills = ['marketing', 'sales', 'entrepreneurship', 'management', 'leadership', 'strategy', 'product management'];
      const creativeSkills = ['design', 'writing', 'content creation', 'video editing', 'animation', 'illustration'];
      const wellnessSkills = ['fitness', 'nutrition', 'yoga', 'meditation', 'coaching', 'mental health'];
      ...
      logger.error(`Error generating Maven opportunities: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  private createTechCourseOpportunity(skills: string[]): RawOpportunity {
    ...
    const relatedSkills = ['programming', 'web development', 'data science', 'machine learning', 'AI', 'python', 'javascript'];
  ...
  private createBusinessCourseOpportunity(skills: string[]): RawOpportunity {
    ...
    const relatedSkills = ['marketing', 'sales', 'entrepreneurship', 'management', 'leadership', 'strategy', 'product management'];
    ...
    return {
      ...
      niceToHaveSkills: ['public speaking', 'community management', 'course creation'],
      estimatedIncome: {
        min: 10000,
        max: 60000,
        timeframe: 'course'
  ...
  private createCreativeCourseOpportunity(skills: string[]): RawOpportunity {
    ...
    const relatedSkills = ['design', 'writing', 'content creation', 'video editing', 'animation', 'illustration'];
    ...
    return {
      ...
      niceToHaveSkills: ['public speaking', 'curriculum design', 'feedback delivery'],
      estimatedIncome: {
        min: 3000,
        max: 30000,
        timeframe: 'course'
  ...
  private createWellnessCourseOpportunity(skills: string[]): RawOpportunity {
    ...
    const relatedSkills = ['fitness', 'nutrition', 'yoga', 'meditation', 'coaching', 'mental health'];
    ...
    return {
      ...
      niceToHaveSkills: ['group facilitation', 'community building', 'accountability systems'],
      estimatedIncome: {
        min: 4000,
        max: 25000,
        timeframe: 'course'
  ...
  private createGeneralCourseOpportunity(skills: string[]): RawOpportunity {
    ...
    return {
      ...
      platform: 'Maven',
      type: OpportunityType.INFO_PRODUCT,
      requiredSkills: ['teaching', 'expertise in a specific domain', 'curriculum design'],
      niceToHaveSkills: ['public speaking', 'community management', 'marketing'],
      estimatedIncome: {
        min: 3000,
        max: 30000,
        timeframe: 'course'
