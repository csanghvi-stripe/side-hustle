...
Do not include any explanatory text before or after the JSON array.`,
    ...
    opportunityGenerationJsonFocus: {
      ...
      version: 1,
      description:
        "JSON-focused opportunity generation prompt with stronger validation",
...
IMPORTANT: Only output valid JSON. NO MARKDOWN. NO INTRODUCTIONS. NO EXPLANATIONS. JSON ARRAY ONLY.`,
  ...
  constructor() {
    ...
    logger.info(
      `Initialized prompt manager with templates for types: ${templateTypes.join(", ")}`,
    );
  }
  ...
  public trackSuccess(
    ...
    if (template) {
      ...
      const newSuccessRate =
        (previousSuccesses + 1) /
        (previousSuccesses + 1 + (this.errorCounts[templateId] || 0));
      ...
      logger.info(
        `Tracked successful use of template: ${templateId}, new success rate: ${newSuccessRate.toFixed(2)}`,
      );
    }
  }
  ...
  public trackError(type: string, templateId: string, error: string): void {
    ...
    logger.warn(
      `Tracked error for template ${templateId}: ${error}, count: ${this.errorCounts[templateId]}`,
    );
  ...
  private createAlternativeTemplate(
    ...
    if (!original) {
      logger.warn(
        `Cannot create alternative for non-existent template: ${originalId}`,
      );
      return;
    }
    ...
    logger.info(
      `Created alternative template: ${newTemplate.id} based on error patterns`,
    );
  }
}
