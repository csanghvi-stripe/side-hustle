      ...
      >
        <CardHeader className="p-4 pb-0">
          <div className="flex justify-between items-start mb-2">
            <Badge
              variant="outline"
              className={`${getBadgeStyle(type)} mr-2 flex items-center gap-1`}
            >
              {getIconForType(type)}
            ...
            <Badge
              variant="outline"
              className={getPriorityBadgeStyle(priority)}
            >
              {priority}
            </Badge>
          </div>
          ...
          <div className="flex items-center mt-1 gap-2">
            ...
            <div className="flex items-center">
              ...
              <span className="text-xs">
                {incomePotential || "$500-1000/month"}
              </span>
            </div>

            <Badge
              variant="outline"
              className={`${getRiskBadgeStyle(riskLevel)} text-xs`}
            >
...
export default function InspirePage() {
  ...
  const getPriorityForOpportunity = (opportunity: any): string => {
    ...
    try {
      data =
        ...
        typeof opportunity.opportunityData === "string"
  ...
  const getFilteredOpportunities = () => {
    if (
      ...
      !Array.isArray(results.opportunities)
    ) {
      return [];
    }

    return results.opportunities.filter((opportunity: any) => {
      const priority = getPriorityForOpportunity(opportunity)
        .toLowerCase()
        .replace(" ", "-");

      if (activeFilter === "all") return true;
      if (activeFilter === "quick-wins" && priority === "quick-win")
        return true;
      if (activeFilter === "growth" && priority === "growth-opportunity")
        return true;
      if (activeFilter === "aspirational" && priority === "aspirational-path")
        return true;
      if (activeFilter === "passive" && priority === "passive-income")
        return true;
      ...
      <div className="container mx-auto py-8 px-4 sm:px-6">
        <div className="text-center p-8 border rounded-lg">
          ...
          <p className="text-neutral-600 mb-6">
            We couldn't find any monetization opportunities matching your search
            criteria.
          </p>
          <Button asChild>
            <Link href="/">
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-neutral-600 mt-2">
            Discover ways to monetize your skills based on your profile and
            preferences.
          </p>
        </div>
          ...
          <div className="flex items-start">
            ...
            <div>
              <h3 className="font-medium text-lg mb-1">
                We've found {results.opportunities.length} opportunity matches
                for you
              </h3>
              <p className="text-neutral-700">
                These opportunities are matched to your skills in{" "}
                {results.userProfile?.skills?.join(", ")}
                and your preference for {
                  results.userProfile?.timeAvailability
                }{" "}
                time commitment with {results.userProfile?.riskTolerance} risk
                tolerance.
              </p>
            </div>
          </div>
